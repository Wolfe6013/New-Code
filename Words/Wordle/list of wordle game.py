...
def GreenAndYellow():
    ...
    for Word in WordList:
        ...
        for x, Letter in enumerate(Word):
            if Letter == Wordle[x] or Wordle[x] in AcceptableVoid: MatchLevel += 1
        if MatchLevel == 5: Possible1Words.append(Word)
    PossibleWords = []
    ActualWords = []
    AcceptableVoid = ['_','-',' ','.']
    Wordle = input('Yellow Letters: ')
    for Word in Possible1Words:
        if Wordle[0] != Word[0]:
            if Wordle[1] != Word[1]:
                if Wordle[2] != Word[2]:
                    if Wordle[3] != Word[3]:
...
while True:
    Found = 1
    SearchType = input('Green, Yellow, Black, BothGY, BothGB, BothYB or All? ')
    if SearchType == 'Green':
        Found = 0
        GreenLetters()             ###Only Green
    if SearchType == 'Yellow':
        Found = 0
        YellowLetters()            ###Only Yellow
    if SearchType == 'Black':
        Found = 0
        BlackLetters()            ###Only Black
    if SearchType == 'BothGY':
        Found = 0
        GreenAndYellow()           ###Both Green and Yellow Combined
    if SearchType == 'BothGB':
        Found = 0
        GreenAndBlack()           ###Both Green and Black Combined
    if SearchType == 'BothYB':
        Found = 0
        YellowAndBlack()           ###Both Yellow and Black Combined
    if SearchType == 'All':
        Found = 0
        GreenYellowBlack()         ###All Green, Yellow and Black
    if Found == 1:
        print(f"{SearchType} isn't an option")
    print()